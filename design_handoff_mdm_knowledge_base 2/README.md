...
A speech bubble + companion bubble. References the chat product directly. Two rounded forms: a larger one with a tail (the "host"), and a smaller one alongside (the "companion / sidecar").

```jsx
<svg viewBox="0 0 140 92" fill="none">
  {/* big bubble with tail */}
  <path d="
...
The mark renders cleanly at all sizes including 16px favicon. Use it in:
- The sidebar workspace header (small, ~22px, inside a gradient rounded-square chip)
- Assistant-message avatar (small, ~18px, inside the gradient avatar tile)
- Favicon / app icon
- Marketing surfaces (large, standalone)
...
│                    #   defines :root tokens, mounts React
...
├── ui.jsx           # Icon, Pill, IconButton, Avatar, DocTypeBadge,
│                    #   SidecarMark, SidecarWordmark primitives
