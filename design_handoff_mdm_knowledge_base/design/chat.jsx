...
const ChatHeader = ({ onToggleInspector, inspectorOpen, onToggleSidebar, sidebarCollapsed }) => {
  const m = window.MDM_DATA.activeMessages.find(x => x.role === 'assistant');
  return (
    <div style={{
      flexShrink: 0,
      ...
      {m && m.vendor ? (
        <div style={{
          ...
          <HierarchyChain vendor={m.vendor} product={m.product} domain={m.domain} extension={m.extension}/>
        </div>
      ) : null}
    </div>
  );
...
const AssistantMessage = ({ m, onOpenSource }) => {
  ...
  return (
    <div style={{ display: 'flex', padding: '12px 0', gap: 12 }}>
      {/* Bot avatar — Sidecar friendly face (Concept 4) */}
      <div style={{
        ...
        boxShadow: '0 1px 0 rgba(15,22,36,0.04), 0 4px 12px -4px oklch(0.65 0.17 35 / 0.25)',
      }}>
        <SidecarMarkFace size={26} bubble="oklch(0.65 0.17 35)" feature="oklch(0.20 0.018 50)" whites="oklch(0.985 0.010 80)"/>
      </div>

      <div style={{ flex: 1, minWidth: 0 }}>
        {/* Header line */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
          <SidecarWordmark size={14} color="var(--fg)" accent="oklch(0.65 0.17 35)"/>
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
              ...
              {c.doc.vendor ? <VendorPill vendor={c.doc.vendor} level={1} mono>{VENDORS[c.doc.vendor].name}</VendorPill> : null}
  ...
  return (
    <div style={{ padding: '0 0 18px' }}>
      ...
      <div style={{ textAlign: 'center', fontSize: 11, color: 'var(--fg-4)', marginTop: 8 }}>
        Sidecar is always by your side — but verify before applying to production records.
      </div>
    </div>
  );
};
