...
/** "02-informatica-mdm" → "Informatica MDM"-ish section label. */
const sectionLabel = (s: string) =>
  s.replace(/^\d+[-_]?/, "").replace(/[-_]/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()) || "Other";

/** Browse + read curated wiki pages: section-grouped list on the left, rendered page on the right. */
export function WikiBrowser() {
  ...
  return (
    <div style={{ display: "flex", gap: 14, height: "70vh" }}>
      {/* list */}
      <div style={{ width: 290, flexShrink: 0, display: "flex", flexDirection: "column", borderRight: "1px solid var(--border)", paddingRight: 12, minHeight: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", marginBottom: 8, background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 8, boxShadow: "var(--shadow-sm)" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", minWidth: 0 }}>
        {!sel && !loading && (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, color: "var(--fg-4)", textAlign: "center", padding: 24 }}>
            <Icon name="wiki" size={26} />
            <div style={{ fontSize: 13 }}>Select a page to read.</div>
            <div style={{ fontSize: 11.5, maxWidth: 320 }}>Wiki pages are curated, authored knowledge — the same content the assistant answers from.</div>
          </div>
        )}
        {loading && <div style={{ fontSize: 12.5, color: "var(--fg-4)", padding: 8 }}>Loading…</div>}
        {sel && !loading && (
          <article>
            <h1 style={{ fontSize: 20, fontWeight: 700, color: "var(--fg)", margin: "0 0 8px" }}>{sel.title}</h1>
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center", marginBottom: 14, paddingBottom: 12, borderBottom: "1px solid var(--border)" }}>
              ...
              {sel.updated && <span style={{ fontSize: 11, color: "var(--fg-4)", marginLeft: "auto" }}>Updated {sel.updated}</span>}
            </div>
